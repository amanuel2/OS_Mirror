...
extern void add_char_val_term(uint8_t data);
...
{     
    if(shift == true)
    {
      if(scancode != 11)
      {
        add_char_val_term((number_shift[(((int)scancode)-2)]));
        printf("%c" , (number_shift[(((int)scancode)-2)]));
      }
      else
      {
        add_char_val_term(number_shift[9]);
    ...
    else
    {
      if(scancode != 11)
      {
        add_char_val_term( '0' + (((int)scancode)-1) );
